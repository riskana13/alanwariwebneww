"use client";

import { useEffect, useState, useCallback } from "react";
import { createClient } from "@/utils/supabase/client";
import { useRouter } from "next/navigation";
import DashboardLayout from "@/app/components/DashboardLayout";

export default function MapelPage() {
  const supabase = createClient();

  // State untuk form input
  const [nama, setNama] = useState("");
  const [guruId, setGuruId] = useState("");
  const [kelasId, setKelasId] = useState("");
  const [jamMulai, setJamMulai] = useState("");
  const [jamSelesai, setJamSelesai] = useState("");
  const [editId, setEditId] = useState(null);

  // Data dari database
  const [mapelList, setMapelList] = useState([]);
  const [guruList, setGuruList] = useState([]);
  const [kelasList, setKelasList] = useState([]);
  const [loading, setLoading] = useState(true);

  // --- Fetch Data ---
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      // Fetch guru
      const { data: guruData, error: guruError } = await supabase
        .from("guru")
        .select("id, nama")
        .order("nama", { ascending: true });

      if (guruError) {
        console.error("Error guru:", guruError);
        setGuruList([]);
      } else {
        setGuruList(guruData || []);
      }

      // Fetch kelas
      const { data: kelasData, error: kelasError } = await supabase
        .from("kelas")
        .select("id, kelas_id, kelas")
        .order("kelas_id", { ascending: true });

      if (kelasError) {
        console.error("Error kelas:", kelasError);
        setKelasList([]);
      } else {
        setKelasList(kelasData || []);
      }

      // Fetch mata pelajaran
      const { data: mapelData, error: mapelError } = await supabase
        .from("mata_pelajaran")
        .select("id, nama, guru_id, kelas_id, jam_mulai, jam_selesai")
        .order("nama", { ascending: true });

      if (mapelError) {
        console.error("Error mapel:", mapelError);
        setMapelList([]);
      } else {
        // Map dengan nama guru dan kelas
        const mapelWithRelations = (mapelData || []).map((m) => {
          const guru = guruData?.find((g) => String(g.id) === String(m.guru_id));
          const kelas = kelasData?.find((k) => String(k.id) === String(m.kelas_id));
          return {
            ...m,
            guru_nama: guru?.nama || "-",
            kelas_nama: kelas ? kelas.kelas : "-",
          };
        });
        setMapelList(mapelWithRelations);
      }
    } catch (error) {
      console.error("Error fetching data:", error);
    } finally {
      setLoading(false);
    }
  }, [supabase]);

  useEffect(() => {
    fetchData();
  }, [fetchData]);

  // --- Tambah / Update Data ---
  const handleSubmit = async (e) => {
    e.preventDefault();

    if (!nama || !guruId || !kelasId || !jamMulai || !jamSelesai) {
      alert("Semua field wajib diisi!");
      return;
    }

    let result;

    try {
      if (editId) {
        // UPDATE
        result = await supabase
          .from("mata_pelajaran")
          .update({ 
            nama: nama.trim(), 
            guru_id: guruId,
            kelas_id: kelasId,
            jam_mulai: jamMulai,
            jam_selesai: jamSelesai
          })
          .eq("id", editId)
          .select("*");
      } else {
        // INSERT
        result = await supabase
          .from("mata_pelajaran")
          .insert([{ 
            nama: nama.trim(), 
            guru_id: guruId,
            kelas_id: kelasId,
            jam_mulai: jamMulai,
            jam_selesai: jamSelesai
          }])
          .select("*");
      }

      if (result.error) {
        alert("Error: " + result.error.message);
        return;
      }

      alert(editId ? "Data berhasil diupdate" : "Data berhasil ditambahkan");

      // Refresh data dengan mapping
      const newData = result.data[0];
      const guru = guruList.find((g) => String(g.id) === String(guruId));
      const kelas = kelasList.find((k) => String(k.id) === String(kelasId));

      const mappedData = {
        ...newData,
        guru_nama: guru?.nama || "-",
        kelas_nama: kelas ? `Kelas ${kelas.kelas_id}` : "-",
      };

      if (editId) {
        setMapelList(
          mapelList.map((m) =>
            m.id === editId ? mappedData : m
          )
        );
      } else {
        setMapelList([...mapelList, mappedData]);
      }

      // Reset Form
      resetForm();
      
    } catch (error) {
      alert("Terjadi kesalahan: " + error.message);
    }
  };

  // --- Reset Form ---
  const resetForm = () => {
    setNama("");
    setGuruId("");
    setKelasId("");
    setJamMulai("");
    setJamSelesai("");
    setEditId(null);
  };

  // --- Hapus Data ---
  const handleDelete = async (id) => {
    if (!confirm("Hapus mata pelajaran ini?")) return;

    try {
      const { error } = await supabase.from("mata_pelajaran").delete().eq("id", id);

      if (error) {
        alert("Gagal menghapus data: " + error.message);
        return;
      }

      setMapelList(mapelList.filter((m) => m.id !== id));
      alert("Data berhasil dihapus");
    } catch (error) {
      alert("Terjadi kesalahan: " + error.message);
    }
  };

  // --- Edit Data (prefill form) ---
  const handleEdit = (m) => {
    setEditId(m.id);
    setNama(m.nama);
    setGuruId(String(m.guru_id));
    setKelasId(String(m.kelas_id));
    setJamMulai(m.jam_mulai || "");
    setJamSelesai(m.jam_selesai || "");
  };

  return (
    <DashboardLayout>
      <h1 className="text-2xl sm:text-3xl font-bold text-teal-800 mb-4 sm:mb-6">Data Mata Pelajaran</h1>

        {loading ? (
          <div className="flex justify-center items-center py-8">
            <p className="text-lg">Memuat data...</p>
          </div>
        ) : (
          <>
            {/* FORM */}
            <form
              onSubmit={handleSubmit}
              className="bg-white p-4 sm:p-6 rounded-xl shadow-md border mb-6 sm:mb-8"
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                  <label className="block text-gray-700 mb-2">Nama Mata Pelajaran</label>
                  <input
                    type="text"
                    value={nama}
                    onChange={(e) => setNama(e.target.value)}
                    className="w-full border px-3 py-2 rounded focus:outline-none focus:ring-2 focus:ring-teal-500"
                    placeholder="Masukkan nama mapel"
                    required
                  />
                </div>

                <div>
                  <label className="block text-gray-700 mb-2">Guru Pengampu</label>
                  <select
                    value={guruId}
                    onChange={(e) => setGuruId(e.target.value)}
                    className="w-full border px-3 py-2 rounded focus:outline-none focus:ring-2 focus:ring-teal-500"
                    required
                  >
                    <option value="">Pilih Guru</option>
                    {guruList.map((g) => (
                      <option key={g.id} value={g.id}>
                        {g.nama}
                      </option>
                    ))}
                  </select>
                </div>

                <div>
                  <label className="block text-gray-700 mb-2">Kelas</label>
                  <select
                    value={kelasId}
                    onChange={(e) => setKelasId(e.target.value)}
                    className="w-full border px-3 py-2 rounded focus:outline-none focus:ring-2 focus:ring-teal-500"
                    required
                  >
                    <option value="">Pilih Kelas</option>
                    {kelasList.map((k) => (
                      <option key={k.id} value={k.id}>
                        {k.kelas}
                      </option>
                    ))}
                  </select>
                </div>

                <div>
                  <label className="block text-gray-700 mb-2">Jam Mulai</label>
                  <input
                    type="time"
                    value={jamMulai}
                    onChange={(e) => setJamMulai(e.target.value)}
                    className="w-full border px-3 py-2 rounded focus:outline-none focus:ring-2 focus:ring-teal-500"
                    required
                  />
                </div>

                <div>
                  <label className="block text-gray-700 mb-2">Jam Selesai</label>
                  <input
                    type="time"
                    value={jamSelesai}
                    onChange={(e) => setJamSelesai(e.target.value)}
                    className="w-full border px-3 py-2 rounded focus:outline-none focus:ring-2 focus:ring-teal-500"
                    required
                  />
                </div>
              </div>

              <div className="flex gap-2 mt-4">
                <button
                  type="submit"
                  className="bg-teal-700 hover:bg-teal-800 text-white px-6 py-2 rounded transition duration-200"
                >
                  {editId ? "Update Data" : "Tambah Mapel"}
                </button>

                {editId && (
                  <button
                    type="button"
                    onClick={resetForm}
                    className="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded transition duration-200"
                  >
                    Batal Edit
                  </button>
                )}
              </div>
            </form>

            {/* TABLE */}
            <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md border">
              <h2 className="text-base sm:text-lg font-semibold mb-4">Daftar Mata Pelajaran</h2>

              {mapelList.length > 0 ? (
                <div className="overflow-x-auto -mx-4 sm:mx-0">
                  <div className="min-w-full inline-block align-middle">
                    <table className="w-full border-collapse border text-xs sm:text-sm">
                      <thead>
                        <tr className="bg-teal-700 text-white">
                          <th className="py-2 sm:py-3 px-2 sm:px-4 border text-left">No</th>
                          <th className="py-2 sm:py-3 px-2 sm:px-4 border text-left">Nama Mapel</th>
                          <th className="py-2 sm:py-3 px-2 sm:px-4 border text-left">Guru Pengampu</th>
                          <th className="py-2 sm:py-3 px-2 sm:px-4 border text-left">Kelas</th>
                          <th className="py-2 sm:py-3 px-2 sm:px-4 border text-left">Jam</th>
                          <th className="py-2 sm:py-3 px-2 sm:px-4 border text-center">Aksi</th>
                        </tr>
                      </thead>

                      <tbody>
                        {mapelList.map((m, i) => (
                          <tr key={m.id} className="border-b hover:bg-gray-50">
                            <td className="py-2 sm:py-3 px-2 sm:px-4 border">{i + 1}</td>
                            <td className="py-2 sm:py-3 px-2 sm:px-4 border">{m.nama}</td>
                            <td className="py-2 sm:py-3 px-2 sm:px-4 border">{m.guru_nama}</td>
                            <td className="py-2 sm:py-3 px-2 sm:px-4 border">{m.kelas_nama}</td>
                            <td className="py-2 sm:py-3 px-2 sm:px-4 border whitespace-nowrap">{m.jam_mulai} - {m.jam_selesai}</td>

                          <td className="py-2 sm:py-3 px-2 sm:px-4 border text-center">
                            <div className="flex gap-1 sm:gap-2 justify-center flex-wrap">
                              <button
                                onClick={() => handleEdit(m)}
                                className="px-3 py-1 rounded bg-yellow-500 hover:bg-yellow-600 text-white text-sm transition duration-200"
                              >
                                Edit
                              </button>

                              <button
                                onClick={() => handleDelete(m.id)}
                                className="px-3 py-1 rounded bg-red-600 hover:bg-red-700 text-white text-sm transition duration-200"
                              >
                                Hapus
                              </button>
                            </div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                </div>
                </div>
              ) : (
                <div className="text-center py-8 text-gray-500">
                  <p>Belum ada data mata pelajaran.</p>
                </div>
              )}
            </div>
          </>
        )}
    </DashboardLayout>
  );
}
